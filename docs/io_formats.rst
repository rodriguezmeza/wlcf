...
``rootDir`` stores the angular 3PCF and bispectrum tables. The default is
``Output``. Also stores run-level outputs such as logs and used-parameter files.

Most files written to ``rootDir`` use the configured ``prefix``. With the
